/** Shared list/add/edit/remove UI for an academy's branches - used by both the manager's and
 * the superadmin's branch tools, which differ only in which CRUD functions they pass in.
 */
...
interface BranchManagerProps {
  branches: Branch[];
  onAdd: (data: BranchFormValues) => Promise<unknown>;
  onUpdate: (id: number, data: BranchFormValues) => Promise<unknown>;
  onDelete: (id: number) => Promise<unknown>;
  /** Called after any successful add/edit/delete, so the caller can refetch the branch list. */
  onChanged: () => void;
}

export function BranchManager({ branches, onAdd, onUpdate, onDelete, onChanged }: BranchManagerProps): ReactElement {
  ...
  function handleAdd(event: FormEvent<HTMLFormElement>): void {
    ...
    setBusy(true);
    setActionError(null);
    onAdd({ name, address: newForm.address.trim(), google_maps_url: newForm.google_maps_url.trim() })
      .then(() => {
