...
interface SelectFieldProps extends SelectHTMLAttributes<HTMLSelectElement> {
  ...
  /** Text of the empty first option (value ""); defaults to a translated "Select…". Pass null for no empty option. */
...
export function SelectField({
  ...
  const resolvedPlaceholder = placeholder === null ? null : (placeholder ?? t("common:select.placeholder"));
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="text-xs font-bold uppercase tracking-wider text-black/80">
        {label}
        {optional && (
          <span className="ms-1 font-medium normal-case tracking-normal text-black/40">
